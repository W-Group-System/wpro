    ...
    public function collection()
    {
        ...
        foreach ($grouped as $employee_no => $rows) {
            ...
            $subtotal = [
                ...
                'in'             => null, 
                'out'            => null,
                'abs'               => (int) ($rows->sum('abs') ?? 0),
                'lv_w_pay'          => (int) ($rows->sum('lv_w_pay') ?? 0),
                'reg_hrs'           => (int) ($rows->sum('reg_hrs') ?? 0),
                'late_min'          => (int) ($rows->sum('late_min') ?? 0),
                'undertime_min'     => (int) ($rows->sum('undertime_min') ?? 0),
                'reg_ot'            => (int) ($rows->sum('reg_ot') ?? 0),
                'reg_nd'            => (int) ($rows->sum('reg_nd') ?? 0),
                'reg_ot_nd'         => (int) ($rows->sum('reg_ot_nd') ?? 0),
                'rst_ot'            => (int) ($rows->sum('rst_ot') ?? 0),
                'rst_ot_over_eight' => (int) ($rows->sum('rst_ot_over_eight') ?? 0),
                'rst_nd'            => (int) ($rows->sum('rst_nd') ?? 0),
                'rst_nd_over_eight' => (int) ($rows->sum('rst_nd_over_eight') ?? 0),
                'lh_ot'             => (int) ($rows->sum('lh_ot') ?? 0),
                'lh_ot_over_eight'  => (int) ($rows->sum('lh_ot_over_eight') ?? 0),
                'lh_nd'             => (int) ($rows->sum('lh_nd') ?? 0),
                'lh_nd_over_eight'  => (int) ($rows->sum('lh_nd_over_eight') ?? 0),
                'sh_ot'             => (int) ($rows->sum('sh_ot') ?? 0),
                'sh_ot_over_eight'  => (int) ($rows->sum('sh_ot_over_eight') ?? 0),
                'sh_nd'             => (int) ($rows->sum('sh_nd') ?? 0),
                'sh_nd_over_eight'  => (int) ($rows->sum('sh_nd_over_eight') ?? 0),
                'rst_lh_ot'         => (int) ($rows->sum('rst_lh_ot') ?? 0),
                'rst_lh_ot_over_eight' => (int) ($rows->sum('rst_lh_ot_over_eight') ?? 0),
                'rst_lh_nd'         => (int) ($rows->sum('rst_lh_nd') ?? 0),
                'rst_lh_nd_over_eight' => (int) ($rows->sum('rst_lh_nd_over_eight') ?? 0),
                'rst_sh_ot'         => (int) ($rows->sum('rst_sh_ot') ?? 0),
                'rst_sh_ot_over_eight' => (int) ($rows->sum('rst_sh_ot_over_eight') ?? 0),
                'rst_sh_nd'         => (int) ($rows->sum('rst_sh_nd') ?? 0),
                'rst_sh_nd_over_eight' => (int) ($rows->sum('rst_sh_nd_over_eight') ?? 0),
                'remarks'        => "",
            ];
