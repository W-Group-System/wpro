    ...
    public function ytd_report(Request $request)
    {
        ...
        if ($from_date) { 
            ...
            $salary_adjustments = SalaryAdjustment::select('name')
                ->whereIn('pay_reg_id', $emp_data->pluck('id')->toArray())
                ->groupBy('name')
                ->orderBy('name', 'ASC')
                ->get();

            $salary_adjustments_data = SalaryAdjustment::whereIn('pay_reg_id', $emp_data->pluck('id')->toArray())->get();
            ...
            if (Schema::hasTable('thirteenth_month_postings')) {
                ...
                $employeeCodes = collect($employeeCodes)
                    ->filter()
                    ->unique()
                    ->values()
                    ->toArray();
