...
class AttendanceController extends Controller
{
    ...
    public function store_logs(Request $request)
    {
       ...
       foreach($request->data as $req)
       {
            ...
            $attendance->datetime = date('Y') . date('-m-d', strtotime($req['timestamp']));
    ...
    public function getlastIdHK($company)
    {
        ...
        $attendance = AttendanceLog::Where('ip_address',$company)->whereNotNull('last_id')->orderBy('id','desc')->first();
        // $id = $attendance->last_id;
        if($attendance != null)
        {
        ...
        foreach ($employees as $employee_code => $dates) {
            foreach ($dates as $date => $employee) {
                AttendanceDetailedReport::create([
                    ...
                    'reg_ot' => $employee['reg_ot'] ?? 0.00,
                    'reg_nd' => $employee['reg_nd'] ?? 0.00,
                    'reg_ot_nd' => $employee['reg_ot_nd'] ?? 0.00,
                    'rst_ot' => $employee['rst_ot'] ?? 0.00,
                    'rst_ot_over_eight' => $employee['rst_ot_over_eight'] ?? 0.00,
                    'rst_nd' => $employee['rst_nd'] ?? 0.00,
                    'rst_nd_over_eight' => $employee['rst_nd_over_eight'] ?? 0.00,
                    'lh_ot' => $employee['lh_ot'] ?? 0.00,
                    'lh_ot_over_eight' => $employee['lh_ot_over_eight'] ?? 0.00,
                    'lh_nd' => $employee['lh_nd'] ?? 0.00,
                    'lh_nd_over_eight' => $employee['lh_nd_over_eight'] ?? 0.00,
                    'sh_ot' => $employee['sh_ot'] ?? 0.00,
                    'sh_ot_over_eight' => $employee['sh_ot_over_eight'] ?? 0.00,
                    'sh_nd' => $employee['sh_nd'] ?? 0.00,
                    'sh_nd_over_eight' => $employee['sh_nd_over_eight'] ?? 0.00,
