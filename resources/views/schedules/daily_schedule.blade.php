                    ...
                    <div class="card-body">
                        ...
                        <p class="card-description">
                          @if (checkUserPrivilege('settings_add',auth()->user()->id) == 'yes')
                            <button type="button" class="btn btn-outline-warning btn-icon-text" data-toggle="modal" data-target="#uploadSchedule">
                            ...
                            <div class="col-md-3">
                              ...
                              <select name="employee" class="form-control js-example-basic-single" required>
                                ...
                                @foreach ($employee as $emp)
                                  <option value="{{$emp->employee_number}}" {{$empNum==$emp->employee_number?'selected':''}}>{{$emp->employee_code}}-{{$emp->first_name.' '.$emp->last_name}}</option>
                                @endforeach
                              </select>
                            </div>
                            <div class="col-md-3">
                          ...
                          <tbody>
                            @foreach ($dailySchedule as $ds)
                              <tr>
                                ...
                                <td>{{$ds->time_in_from != "00:00" ? date('h:i A', strtotime($ds->time_in_from)) : 'Rest Day'}}</td>
                                <td>{{$ds->time_in_to != "00:00" ? date('h:i A', strtotime($ds->time_in_to)) : 'Rest Day'}}</td>
                                <td>{{$ds->time_out_from != "00:00" ? date('h:i A', strtotime($ds->time_out_from)) : 'Rest Day'}}</td>
                                <td>{{$ds->time_out_to != "00:00" ? date('h:i A', strtotime($ds->time_out_to)) : 'Rest Day'}}</td>
                                <td>{{$ds->working_hours!=null?$ds->working_hours:'0.00'}}</td>
                                {{-- <td>
                                  <button type="button" class="btn btn-outline-info btn-icon-text btn-sm" data-toggle="modal" data-target="#editDailySchedule-{{$ds->id}}">
                                    Edit
