                                ...
                                <tr>
                                    ...
                                    <td id="tdStatus{{ $item->id }}">
                                        ...
                                        @if($item->approver_by)
                                            @if($item->status == 'Approved')
                                                @if($item->approveBy)
                                                    {{$item->approveBy->name ?? 'N/A'}} -  <label class="badge badge-success mt-1">Approved</label>
                                                @endif
                                            @elseif($item->status == 'Pending')
                                                @if($item->approveBy)
                                                    {{$item->approveBy->name ?? 'N/A'}} -  <label class="badge badge-warning mt-1">Pending</label>
                                                @endif
                                            @elseif($item->status == 'Declined')
                                                @if($item->approveBy)
                                                    {{$item->approveBy->name ?? 'N/A'}} -  <label class="badge badge-danger mt-1">Declined</label>
                                                @endif
                                            @endif
                                        {{-- @else
                                        <label class="badge badge-danger mt-1">No Approver</label> --}}
